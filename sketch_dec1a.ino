...
uint8_t lock_led_high = 4; // пиы верхнего и нижнего светодиодов
uint8_t lock_led_low = 5; 
uint8_t But_save = 6;  // кнопка на запись нового мотора
...
Motor motor_lock(lock_led_high, lock_led_low, photo_high, photo_low, forward_pin, back_pin, 100);
...
void setup() {
  ...
  pinMode(But_save, INPUT_PULLUP);

  pinMode(forward_pin, OUTPUT);    // Устанавливаем FwdPin_A как выход
  pinMode(back_pin, OUTPUT);    // Устанавливаем BwdPin_A как выход
